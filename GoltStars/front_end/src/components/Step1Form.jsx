...
export default function Step1Form({ onNext }) {
  ...
  const styles = {
    ...
    input: (index) => ({
      ...
      borderRadius: "8px",
      fontSize: "16px",
      boxShadow: glowIndex === index ? "0 0 15px rgba(52, 152, 219, 0.6)" : "0 2px 5px rgba(0,0,0,0.1)",
  ...
  return (
    <div style={styles.container}>
      ...
      <form onSubmit={handleSubmit}>
        {['nom', 'prenom', 'email', 'adresse'].map((field, index) => (
          <input
            key={field}
            style={styles.input(index)}
            placeholder={
              field === 'nom' ? 'Votre nom' : 
              field === 'prenom' ? 'Votre prénom' :
              field === 'email' ? 'email@exemple.com' : 'Adresse complète'
            }
